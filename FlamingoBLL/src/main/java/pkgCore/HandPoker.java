...
public class HandPoker extends Hand {
	...
	public boolean isFourOfAKind() {
		boolean bisFourOfAKind = false;
	        ...
	        if ((getCRC().size() == 2) && 
	                (getCRC().get(0).getiCnt() == 4) &&
	                (getCRC().get(1).getiCnt() == 1)) {
	...
	public boolean isTwoPair() {
		...
		if ((getCRC().size() == 3) && 
            ...
            HS.setHiCard(getCards().get(getCRC().get(0).getiCardPosition())); // High card
            HS.setLoCard(null); // Kicker only
            HS.setKickers(FindTheKickers(getCRC()));    
        }
	...
	public boolean isPair() {
		...
		if ((getCRC().size() == 4) && 
                ...
                (getCRC().get(1).getiCnt() == 1) &&
                (getCRC().get(2).getiCnt() == 1) &&
                ((getCRC().get(3).getiCnt() == 1))) {
            
            bisPair = true;            
            HandScorePoker HS = getHSP();
            HS.seteHandStrength(eHandStrength.Pair);
            HS.setHiCard(getCards().get(getCRC().get(0).getiCardPosition()));
            HS.setLoCard(null); // Three kickers
            HS.setKickers(FindTheKickers(getCRC()));    
        }
